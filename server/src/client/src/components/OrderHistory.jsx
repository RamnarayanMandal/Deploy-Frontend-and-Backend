...
const OrderHistory = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${window.location.origin}/panShopLogin/orderHistroy/${panshopOwner_id}`);
  ...
  return (
    <div className="flex">
      ...
      <div className={`fixed right-0 top-0 h-full w-2/5 bg-gray-900 z-50 transform transition-transform ease-in-out duration-300 ${openDrawer ? 'translate-x-0' : 'translate-x-full'}`} style={{ height: '100vh', overflowY: 'auto', scrollbarWidth: 'none', '-ms-overflow-style': 'none' }}>
